...
async function fetchAllVendors(): Promise<Vendor[]> {
  try {
    // Try to get unique vendors from products
    const { data } = await api.get<Product[]>('/products');
    const vendorMap = new Map<number, Vendor>();
    data.forEach(p => {
      if (p.vendorId && p.vendorStoreName) {
        vendorMap.set(p.vendorId, {
          id: p.vendorId,
          storeName: p.vendorStoreName,
        });
      }
    });
...
export default function FavouriteVendorsPage() {
  ...
  // For now, this shows all vendors - favourite functionality can be added later
  ...
  // TODO: Integrate with a favorites store
  const favouriteVendorIds: number[] = [];
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-12">
          <div>
            ...
            <p className="text-slate-400">
              Discover and follow your favorite stores. Get updates on new products and exclusive deals.
            </p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredVendors.map((vendor) => {
              const isFavourite = favouriteVendorIds.includes(vendor.id);
              return (
                <div
                  key={vendor.id}
